...
**New Features:**
- `WorkspaceLayout` - Container for dockable panels
- `WorkspacePanel` - Resizable left panel
...
// Add after canvas, before closing WorkspaceLayout:
<WorkspacePanel
  id="bottom-output"
  title="Output"
  side="bottom"
    ...
    <EditorShell>
      ...
      <WorkspaceLayout ref={layoutRef}>
        {isPanelVisible('left-nav') && (
          <WorkspacePanel id="left-nav" title="Navigator" side="left" defaultSize={280}>
        ...
        {isPanelVisible('right-inspector') && (
          <WorkspacePanel id="right-inspector" title="Inspector" side="right" defaultSize={320}>
            <PanelTabs tabs={inspectorTabs} />
          </WorkspacePanel>
        )}

        {isPanelVisible('bottom-output') && (
          <WorkspacePanel id="bottom-output" title="Output" side="bottom" defaultSize={200}>
...
- [EditorInspector Guide](../components/editor-inspector)
