...
- Forge editor components (EditorShell, DockLayout, etc.)
...
import type { DomainAssistantContract, DomainContextSnapshot } from '@forge/dev-kit/assistant';
import type { Selection } from '@forge/dev-kit';
...
import type { DomainTool, DomainToolContext } from '@forge/dev-kit/assistant';
import type { TaskDoc } from '@forge/types/task';
...
import { Badge } from '@forge/dev-kit/ui/badge';
...
import { ToolUIRegistry } from '@forge/dev-kit/components/tool-ui/assistant-tools';
...
export function TaskEditor() {
  ...
  return (
    <EditorShell editorId="task" title="Tasks" domain="task">
      <EditorToolbar>
        <EditorToolbar.Left>
      ...
      <WorkspaceLayout layoutId="task-layout">
        ...
        <WorkspaceLayout.Right>
          <WorkspacePanel id="chat" title="AI Assistant">
...
```typescript
import { AssistantDevToolsBridge } from '@forge/dev-kit/components/assistant-ui';
...
```typescript
// ❌ BAD: AI has no idea what tasks exist
getContextSnapshot: () => ({ domain: 'task', domainState: {} });

// ✅ GOOD: AI knows current state
getContextSnapshot: () => ({
  domain: 'task',
...
```typescript
// ❌ BAD: AI doesn't know when to use tools
...
```typescript
// ❌ BAD: Unhandled errors crash the tool
execute: async (args) => {
  const result = await createTask(args);
...
```typescript
// ❌ BAD: Contract recreated on every render
const contract = useTaskAssistantContract({ ... });

// ✅ GOOD: Wrapped in useMemo internally
...
- [AI Technical Docs](/docs/ai) - Deep dive into AI architecture
...
- ✅ How to define a domain contract
- ✅ How to create AI tools with JSON Schema parameters
- ✅ How to execute operations and update editor state
- ✅ How to add custom tool rendering
- ✅ How to integrate with your editor
- ✅ Real examples from Character and Dialogue editors
