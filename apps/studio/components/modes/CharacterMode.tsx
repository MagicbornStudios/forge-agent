...
// ---------------------------------------------------------------------------
// CharacterMode — the new-naming migration of CharacterWorkspace
// ---------------------------------------------------------------------------
/**
 * CharacterMode — AI-first character management editor mode.
 *
 * Uses the new editor platform components:
 * - `EditorShell` (replaces `WorkspaceShell`)
 * - `ModeHeader` (replaces `WorkspaceHeader`)
 * - `ModeToolbar` (replaces `WorkspaceToolbar`)
 * - `DockLayout` (replaces `WorkspaceLayoutGrid`) — resizable panels!
 * - `DockPanel` (wraps left/right panels with title + scroll)
 * - `ModeStatusBar` (replaces `WorkspaceStatusBar`)
 * - `ModeOverlaySurface` (replaces `WorkspaceOverlaySurface`)
 */
export function CharacterMode() {
  const editorId = MODE_EDITOR_IDS.character;
  const workspaceTheme = useEditorStore((s) => s.workspaceThemes.character);
  ...
  const showLeftPanel = useSettingsStore((s) =>
    s.getSettingValue('panel.visible.character-left', { workspaceId: 'character', editorId }),
  ) as boolean | undefined;
  const showRightPanel = useSettingsStore((s) =>
    s.getSettingValue('panel.visible.character-right', { workspaceId: 'character', editorId }),
  ) as boolean | undefined;
  ...
  const leftPanel =
    showLeftPanel === false ? undefined : (
      <DockPanel panelId="character-navigator" title="Characters" scrollable={false}>
        <CharacterSidebar
  ...
  const rightPanel =
    showRightPanel === false ? undefined : (
      <DockPanel panelId="character-properties" title="Properties" scrollable>
  ...
  return (
    <NodeDragProvider>
      ...
      >
        ...
        {/* Resizable DockLayout replaces fixed WorkspaceLayoutGrid */}
        <DockLayout
          left={leftPanel}
          main={mainContent}
          right={rightPanel}
