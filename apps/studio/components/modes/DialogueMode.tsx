...
import { WorkspaceInspector } from '@forge/shared/components/workspace';
...
export function DialogueMode() {
  ...
  const workspaceTheme = useEditorStore((s) => s.workspaceThemes.dialogue);
  const editorId = MODE_EDITOR_IDS.dialogue;
  const agentName = useSettingsStore((s) =>
    s.getSettingValue('ai.agentName', { workspaceId: 'dialogue', editorId })
  ) as string | undefined;
  const showAgentName = useSettingsStore((s) =>
    s.getSettingValue('ai.showAgentName', { workspaceId: 'dialogue' })
  ) as boolean | undefined;
  const toolsEnabledSetting = useSettingsStore((s) =>
    s.getSettingValue('ai.toolsEnabled', { workspaceId: 'dialogue', editorId })
  ) as boolean | undefined;
  ...
  const showLeftPanel = useSettingsStore((s) =>
    s.getSettingValue('panel.visible.dialogue-left', { workspaceId: 'dialogue', editorId }),
  ) as boolean | undefined;
  const showRightPanel = useSettingsStore((s) =>
    s.getSettingValue('panel.visible.dialogue-right', { workspaceId: 'dialogue', editorId }),
  ) as boolean | undefined;
  const showBottomPanel = useSettingsStore((s) =>
    s.getSettingValue('panel.visible.dialogue-bottom', { workspaceId: 'dialogue', editorId }),
  ) as boolean | undefined;
  ...
  const inspectorContent =
    showRightPanel === false ? undefined : (
      <DockPanel panelId="dialogue-right" title="Inspector" className="h-full">
  ...
  const bottomPanel =
    bottomDrawerOpen && showBottomPanel !== false ? (
      <DockPanel panelId="dialogue-bottom" title="Workbench" scrollable={false}>
      ...
      >
        ...
        <ModeToolbar className="bg-sidebar border-b border-sidebar-border">
          ...
          <ModeToolbar.Right>
            ...
            <SettingsMenu workspaceId="dialogue" editorId={editorId} />
            <ModeToolbar.Button
        ...
        <DockLayout
          ...
          bottom={bottomPanel}
          viewport={{ editorId, editorType: 'react-flow' }}
        ...
        <ModeOverlaySurface overlays={overlays} activeOverlay={activeOverlay} onDismiss={dismissOverlay} />
      </EditorShell>
    </NodeDragProvider>
  );
}
