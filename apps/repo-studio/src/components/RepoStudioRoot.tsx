...
export function RepoStudioRoot({
  ...
  const effectiveWorkspaceId: RepoWorkspaceId = REPO_WORKSPACE_IDS.includes(activeWorkspaceId)
    ? activeWorkspaceId
    : 'planning';

  const activeLayoutId = React.useMemo(() => getWorkspaceLayoutId(effectiveWorkspaceId), [effectiveWorkspaceId]);
  ...
  const panelSpecs = React.useMemo(() => getWorkspacePanelSpecs(effectiveWorkspaceId), [effectiveWorkspaceId]);
  ...
  React.useEffect(() => {
    const activeInvalid = !REPO_WORKSPACE_IDS.includes(activeWorkspaceId);
    const openInvalid = openWorkspaceIds.some((id) => !REPO_WORKSPACE_IDS.includes(id));
    if (!activeInvalid && !openInvalid) return;
    const filtered = openWorkspaceIds.filter((id): id is RepoWorkspaceId => REPO_WORKSPACE_IDS.includes(id));
  ...
  }, [activeWorkspaceId, openWorkspaceIds, setActiveWorkspace, setOpenWorkspaceIds]);
  ...
  const openForgeAssistant = React.useCallback(() => {
    openWorkspaceTab('assistant');
    setPanelVisibleAndPersist('assistant', true, 'assistant');
  }, [openWorkspaceTab, setPanelVisibleAndPersist]);
  ...
  const ActiveLayout = REPO_WORKSPACE_COMPONENTS[effectiveWorkspaceId];
