...
function forgeAssistantReply(input: {
  prompt: string;
  loopId: string;
  nextAction: string;
  percent: number;
...
export async function runLocalForgeAssistant(input: { body: any; assistantTarget: string }) {
  ...
  const message = forgeAssistantReply({
    prompt,
  ...
  const partId = `forge-assistant-${Date.now()}`;
