...
const SOURCE_EXCLUDES_WITH_DOC_ARCHIVE = [
  ...SOURCE_EXCLUDES,
  "docs/legacy/",
  "apps/docs/content/archive/",
];
...
const renderHelperMatches = searchPattern({
  ...
  roots: ["vendor/repo-studio/apps/repo-studio/src/components/workspaces"],
  extensions: [".ts", ".tsx"],
});
...
const editorTargetMatches = searchPattern({
  ...
  roots: [
    ...
    "vendor/repo-studio/packages/repo-studio",
  ],
  extensions: WORKSPACE_EXTENSIONS,
});
if (editorTargetMatches) {
  fail(
    "Repo Studio contracts must use assistantTarget (editorTarget is forbidden).",
    editorTargetMatches,
  );
}
...
const LEGACY_HARD_CUT_CHECKS = [
  ...
  },
  {
    ...
    message:
      "Legacy assistant prompt key detected. Use assistant.prompts.forgeAssistant.",
  },
  {
  ...
  },
  {
    ...
    message:
      "Legacy JSON proposal fallback detected. SQLite-only storage is required.",
  },
  {
    pattern: LEGACY_UI_FLAG,
    message:
      "Legacy CLI fallback flag detected. Legacy package runtime UI fallback is removed.",
  },
  {
    pattern: LEGACY_FORGE_FALLBACK,
    message:
      "Legacy local Forge fallback detected. Forge must use OpenRouter/proxy only.",
  },
  {
...
const repoStudioCodegenPath =
  "vendor/repo-studio/apps/repo-studio/forge-codegen.config.mjs";
const repoStudioCodegenSource = fs.existsSync(repoStudioCodegenPath)
  ? fs.readFileSync(repoStudioCodegenPath, "utf8")
  : "";
const workspaceFileMapMatch = repoStudioCodegenSource.match(
  /const\s+WORKSPACE_FILE_MAP\s*=\s*\{([\s\S]*?)\};/,
);
...
const builtinStoryAppSpecMatches = searchPattern({
  ...
  roots: ["vendor/repo-studio/apps/repo-studio/src/lib/app-spec.generated.ts"],
  extensions: [".ts"],
});
...
const storyFallbackCatalogMatches = searchPattern({
  ...
  roots: ["vendor/repo-studio/apps/repo-studio/src/lib/workspace-catalog.ts"],
  extensions: [".ts"],
});
...
const storyInstallPromptMatches = searchPattern({
  pattern:
    "showStoryInstallPrompt|extensionRegistryHasStory|Story extension is available from registry but not installed",
  roots: [
    "vendor/repo-studio/apps/repo-studio/src/components/RepoStudioRoot.tsx",
  ],
  extensions: [".tsx"],
});
...
const missingRegistryPaths = registryRequiredPaths.filter(
...
const loopSwitcherMatches = searchPattern({
  ...
  roots: [
    "vendor/repo-studio/apps/repo-studio/src/components/workspaces/PlanningWorkspace.tsx",
...
const removedBuiltinWorkspaceMatches = searchPattern({
  ...
  roots: ["vendor/repo-studio/apps/repo-studio/forge-codegen.config.mjs"],
  extensions: [".mjs"],
});
...
const forbiddenAppLocalWorkspaceFiles = [
  "vendor/repo-studio/apps/repo-studio/src/components/workspaces/StoryWorkspace.tsx",
  "vendor/repo-studio/apps/repo-studio/src/components/workspaces/EnvWorkspace.tsx",
  "vendor/repo-studio/apps/repo-studio/src/components/workspaces/GenericExtensionWorkspace.tsx",
];
const forbiddenAppLocalWorkspaceHits = forbiddenAppLocalWorkspaceFiles.filter(
  (target) => fs.existsSync(path.resolve(target)),
);
...
const removedWorkspaceFocusMenuMatches = searchPattern({
  pattern:
    "Focus\\s+(Commands|Diff|Review Queue)|view-focus-(assistant-ws|diff-ws|review-queue|commands)",
  roots: [
    "vendor/repo-studio/apps/repo-studio/src/lib/app-shell/menu-contributions.ts",
  ],
  extensions: [".ts"],
});
