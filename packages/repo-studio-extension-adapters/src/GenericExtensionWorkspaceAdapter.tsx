...
import { useRepoStudioContextOptional } from "../../../vendor/repo-studio/apps/repo-studio/src/lib/app-shell/RepoStudioContext";
...
export function GenericExtensionWorkspaceAdapter({
  ...
  const activeWorkspaceId = useRepoStudioShellStore(
    (state) => state.route.activeWorkspaceId,
  );
      ...
      >
        <WorkspaceLayout.Main>
          <WorkspaceLayout.Panel
            id="workspace"
            title={extension?.label || "Extension"}
            icon={<LayoutGrid size={14} />}
          >
            <div className="flex h-full items-center justify-center p-4 text-sm text-muted-foreground">
              <div className="max-w-xl space-y-2 text-center">
                <p className="font-medium text-foreground">
                  {extension?.label || "Workspace extension"} loaded.
                </p>
                <p>
                  This extension uses host-rendered panels in this slice. Add
                  extension-specific panels/tooling via manifest + host
                  contracts.
                </p>
              </div>
            </div>
