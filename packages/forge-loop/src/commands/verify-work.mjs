...
import {
  ensureHeadlessEnvReady,
  shouldRunHeadlessEnvGate,
} from "../lib/env-preflight.mjs";
import {
  ensurePhaseDir,
  findPhase,
  listPhasePlanFiles,
...
function evaluateVerification(automatedChecks, responses) {
  ...
  const skippedUat = responses.filter(
    (response) => response.result === "skipped",
  );

  const status =
    failedChecks.length > 0 || failedUat.length > 0
      ? "gaps_found"
      : skippedUat.length > 0
        ? "human_needed"
        : "passed";
...
function buildVerificationMarkdown({
  ...
  const automatedTable = automatedChecks
    .map(
      (check) =>
        `| ${check.command} | ${check.ok ? "PASS" : "FAIL"} | ${check.stderr || check.stdout || "-"} |`,
    )
    .join("\n");
  ...
  const truthsTable = responses
    .map(
      (response, index) =>
        `| ${index + 1} | ${response.truth} | ${response.result.toUpperCase()} | ${response.notes || "-"} |`,
    )
    .join("\n");
...
async function collectUatResponses(phase, truths, options = {}) {
  ...
  for (const truth of truths) {
    ...
    const answer = (
      await askLine(
        `UAT: ${truth}\nType 'pass', 'skip', or describe issue: `,
        "pass",
      )
    ).trim();
...
function shouldRunPlatformBuild(changedFiles) {
  return changedFiles.some(
    (filePath) =>
      filePath.startsWith("vendor/platform/apps/platform/") ||
...
function shouldRunStudioBuild(changedFiles) {
  return changedFiles.some(
    (filePath) =>
      filePath.startsWith("apps/studio/") || filePath.startsWith("packages/"),
  );
}
...
function shouldRunRepoStudioBuild(changedFiles) {
  return changedFiles.some(
    (filePath) =>
      filePath.startsWith("vendor/repo-studio/apps/repo-studio/") ||
      filePath.startsWith("vendor/repo-studio/packages/repo-studio/"),
  );
}
...
export function buildVerificationCommandPlan(config, changedFiles) {
  ...
  if (
    ...
    shouldRunPlatformBuild(changedFiles)
  ) {
    commands.push({
      command: "pnpm",
      args: ["--filter", "@forge/platform", "build"],
    });
  }

  if (
    config?.verification?.build !== false &&
    shouldRunStudioBuild(changedFiles)
  ) {
    commands.push({
      command: "pnpm",
      args: ["--filter", "@forge/studio", "build"],
    });
  }

  if (
    config?.verification?.build !== false &&
    shouldRunRepoStudioBuild(changedFiles)
  ) {
    commands.push({
      command: "pnpm",
      args: ["--filter", "@forge/repo-studio-app", "build"],
    });
  }
  ...
  if (
    ...
    commands.push({
      command: "pnpm",
      args: ["--filter", "@forge/studio", "test", "--", "--runInBand"],
    });
  }
...
export async function runVerifyWork(phaseNumber, options = {}) {
  ...
  const checks = commandPlan.map((item) =>
    runCommand(process.cwd(), item.command, item.args),
  );

  const truths = collectMustHaves(planPaths);
  const responses = await collectUatResponses(phase, truths, {
  ...
  const verificationPath = path.join(
    phaseDir.fullPath,
    `${phase.phaseNumber}-VERIFICATION.md`,
  );
  writeText(
    verificationPath,
  ...
  const promptPath = path.join(
    ...
    `${phase.phaseNumber}-verify-prompt.md`,
  );
  writeText(
    promptPath,
  ...
  if (autoCommit) {
    const commitFiles = [verificationPath, uatPath, promptPath].map((item) =>
      path.relative(process.cwd(), item).replace(/\\/g, "/"),
    );
    const commitResult = commitPaths(
      process.cwd(),
      formatVerificationCommitMessage(phase.phaseNumber),
      commitFiles,
      {
        commitScope,
  ...
  if (
    strictMode &&
    (evaluation.failedChecks.length > 0 || evaluation.failedUat.length > 0)
  ) {
    throw new Error(
