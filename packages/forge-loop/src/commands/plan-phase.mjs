...
function buildStagePrompt(phase) {
  return [
    ...
    `Goal: ${phase.goal || 'N/A'}`,
    'Propose wave ordering, dependency sequence, and must-have validation criteria.',
    'Respond using concise checklist bullets.',
  ].join('\n');
}

function validatePhasePlans(phasePlans) {
...
export async function runPlanPhase(phaseNumber, options = {}) {
  ...
  if (runtime.runnerSelected !== 'prompt-pack') {
    const turn = await runtime.provider.runPlan({
    ...
    taskResults.push({
      taskId: `plan-${phase.phaseNumber}`,
      status: 'completed',
      reason: 'prompt-pack planning flow executed.',
      filesTouched: [],
    });
  }
  ...
  const result = enrichStageResult({
    ...
    nextAction: `forge-loop execute-phase ${phase.phaseNumber}`,
    status: taskResults.every((item) => item.status === 'completed') ? 'ready_for_execute' : 'blocked',
  });
