...
- **Dock layout (rails)**: Left, main, right, bottom are **rails**; each rail can have multiple **panel tabs**. Prefer config-driven `leftPanels`/`mainPanels`/`rightPanels`/`bottomPanels` (arrays of `RailPanelDescriptor`: `id`, `title`, `iconKey?`, `content`). **Declarative registration:** In Studio, use **EditorLayoutProvider** + **EditorRail** + **EditorPanel** (shared) so panels register into the panel registry; **EditorLayout** (Studio) consumes the registry. New editor panels and settings sections should use the declarative components and registries. Legacy `left`/`main`/`right`/`rightInspector`+`rightSettings`/`bottom` remain supported when panels arrays are omitted. Use `DockLayout.Left`, `.Main`, `.Right`, `.Bottom` or props; slot children override props. Each tab content is typically an `EditorDockPanel` (leaf, or with `PanelTabs` for inner tabs). See `components/editor/README.md`. Always provide `viewportId` + `viewportType` via the `viewport` prop.
