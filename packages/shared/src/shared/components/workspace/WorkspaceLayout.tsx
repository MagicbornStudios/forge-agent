...
export interface WorkspaceLayoutLeftProps {
  children?: React.ReactNode;
  /** When true, hides the Dockview tab bar for this rail (Cursor/VS Code style static rail). */
  hideTabBar?: boolean;
}
function WorkspaceLayoutLeft({ children }: WorkspaceLayoutLeftProps) {
  return <>{children}</>;
}
WorkspaceLayoutLeft.displayName = 'WorkspaceLayout.Left';

export interface WorkspaceLayoutMainProps {
  children?: React.ReactNode;
  /** When true, hides the Dockview tab bar for this rail (Cursor/VS Code style static rail). */
  hideTabBar?: boolean;
}
function WorkspaceLayoutMain({ children }: WorkspaceLayoutMainProps) {
  return <>{children}</>;
}
WorkspaceLayoutMain.displayName = 'WorkspaceLayout.Main';

export interface WorkspaceLayoutRightProps {
  children?: React.ReactNode;
  /** When true, hides the Dockview tab bar for this rail (Cursor/VS Code style static rail). */
  hideTabBar?: boolean;
}
function WorkspaceLayoutRight({ children }: WorkspaceLayoutRightProps) {
  return <>{children}</>;
}
...
const SLOT_COMPONENTS: Record<SlotId, React.ComponentType<{ children?: React.ReactNode; hideTabBar?: boolean }>> = {
...
function collectSlotsFromChildren(children: React.ReactNode): {
  ...
  leftHideTabBar: boolean;
  mainHideTabBar: boolean;
  rightHideTabBar: boolean;
} {
  const slots: Partial<Record<SlotId, React.ReactNode>> = {};
  const panelsFromSlots: Partial<Record<SlotId, RailPanelDescriptor[]>> = {};
  let leftHideTabBar = false;
  let mainHideTabBar = false;
  let rightHideTabBar = false;

  React.Children.forEach(children, (child) => {
    if (!React.isValidElement(child)) return;
    const type = child.type as React.ComponentType<{ children?: React.ReactNode; hideTabBar?: boolean }>;
    const childProps = child.props as { children?: React.ReactNode; hideTabBar?: boolean };
    for (const slotId of SLOT_IDS) {
      if (type === SLOT_COMPONENTS[slotId]) {
        if (slotId === 'left') leftHideTabBar = Boolean(childProps.hideTabBar);
        if (slotId === 'main') mainHideTabBar = Boolean(childProps.hideTabBar);
        if (slotId === 'right') rightHideTabBar = Boolean(childProps.hideTabBar);
        const slotChildren = childProps.children;
  ...
  return { slots, panelsFromSlots, leftHideTabBar, mainHideTabBar, rightHideTabBar };
}
...
interface HideRailTabHeadersOptions {
...
function hideRailTabHeaders(api: DockviewReadyEvent['api'], options: HideRailTabHeadersOptions) {
  const { layoutSlots, leftPanels, mainPanels, rightPanels, leftHideTabBar, mainHideTabBar, rightHideTabBar } = options;
  const leftIds = new Set(leftPanels?.length ? leftPanels.map((p) => p.id) : layoutSlots.left != null ? ['left'] : []);
  const mainIds = new Set(mainPanels?.length ? mainPanels.map((p) => p.id) : layoutSlots.main != null ? ['main'] : []);
  const rightIds = new Set(rightPanels?.length ? rightPanels.map((p) => p.id) : layoutSlots.right != null ? ['right'] : []);

  for (const group of api.groups) {
    for (const panel of group.panels) {
      const targetPanel = panel as { id: string; group: { header: { hidden: boolean } } };
      if (leftHideTabBar && leftIds.has(targetPanel.id)) targetPanel.group.header.hidden = true;
      if (mainHideTabBar && mainIds.has(targetPanel.id)) targetPanel.group.header.hidden = true;
      if (rightHideTabBar && rightIds.has(targetPanel.id)) targetPanel.group.header.hidden = true;
    }
  }
}
...
const WorkspaceLayoutRoot = forwardRef<WorkspaceLayoutRef, WorkspaceLayoutProps>(function WorkspaceLayoutComponent(
  ...
  const collected = React.useMemo(
    () =>
      children != null
        ? collectSlotsFromChildren(children)
        : { slots: {}, panelsFromSlots: {}, leftHideTabBar: false, mainHideTabBar: false, rightHideTabBar: false },
    [children]
  );
  ...
  const leftHideTabBar = collected.leftHideTabBar;
  const mainHideTabBar = collected.mainHideTabBar;
  const rightHideTabBar = collected.rightHideTabBar;
  ...
  const slotsRef = useRef({
    ...
    rightPanels,
    bottomPanels,
    leftHideTabBar: false,
    mainHideTabBar: false,
    rightHideTabBar: false,
  });
  slotsRef.current = {
    layoutSlots,
    slotConfig,
  ...
  const onReady = useCallback(
    (event: DockviewReadyEvent) => {
      ...
      const { layoutSlots: slots, slotConfig: config, leftPanels, mainPanels, rightPanels, bottomPanels, leftHideTabBar, mainHideTabBar, rightHideTabBar } = slotsRef.current;
      ...
      /** Hide tab bars for rails when hideTabBar is set (Cursor/VS Code style static rails). */
      hideRailTabHeaders(dockApi, {
        layoutSlots: slots,
        leftPanels,
        mainPanels,
        rightPanels,
  ...
  /** Keep rail headers hidden when hideTabBar is enabled for left/main/right rails. */
  useEffect(() => {
    if (!api) return;
    const { layoutSlots: slots, leftPanels, mainPanels, rightPanels, leftHideTabBar, mainHideTabBar, rightHideTabBar } = slotsRef.current;
    hideRailTabHeaders(api, {
      layoutSlots: slots,
      leftPanels,
      mainPanels,
      rightPanels,
  ...
  }, [api, leftHideTabBar, mainHideTabBar, rightHideTabBar, leftPanels, mainPanels, rightPanels]);
