...
Docs in `_docs/` (and eventually `docs/`) include collapsed agent blocks. To find them:

```bash
rg AGENT_SECTION _docs
```

Each block lives inside `<details>` and contains:
...
Parse the content after the `<!-- AGENT_SECTION -->` comment. Same structure everywhere.
