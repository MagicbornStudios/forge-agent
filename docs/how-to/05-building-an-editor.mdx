...
2. Create the editor component with `EditorShell` + `EditorDockLayout`
...
Compose `EditorShell`, `EditorToolbar`, `EditorDockLayout`, `EditorStatusBar`, `EditorOverlaySurface`. Use the **UI-first slot pattern**:
...
export function MyEditor() {
  return (
    <EditorShell editorId="my-editor" title="My Editor" domain="my-domain" className="bg-canvas">
      <EditorToolbar>
        <EditorToolbar.Left><span className="text-xs font-medium">My Editor</span></EditorToolbar.Left>
        <EditorToolbar.Right>
          <EditorToolbar.Button size="sm" variant="outline">Action</EditorToolbar.Button>
        </EditorToolbar.Right>
      </EditorToolbar>

      <EditorDockLayout layoutId="my-editor-layout" leftDefaultSize={20} rightDefaultSize={25}>
        <EditorDockLayout.Left>
          <EditorDockLayout.Panel id="nav" title="Navigator" icon={<BookOpen size={14} />}>
            <EditorDockPanel panelId="nav-panel" hideTitleBar scrollable>
        ...
        <EditorDockLayout.Main>
          <EditorDockLayout.Panel id="main" title="Main" icon={<LayoutDashboard size={14} />}>
            <EditorDockPanel panelId="main-panel" hideTitleBar scrollable>
        ...
        <EditorDockLayout.Right>
          <EditorDockLayout.Panel id="inspector" title="Inspector" icon={<ScanSearch size={14} />}>
            <EditorDockPanel panelId="inspector-panel" hideTitleBar scrollable>
...
Put your editor UI in `EditorDockLayout.Main`. Hold selection in local state or a store; expose it for the inspector and domain contract. Example from DialogueEditor: selection is `{ type: 'entity', entityType: 'forge.node', id }` or `{ type: 'none' }`. Use `packages/shared/src/shared/workspace/selection.ts` for the shape.

## Step 4: Inspector (optional)

Use `EditorDockLayout.Right` with `EditorDockPanel` wrapping `EditorInspector`. Pass `selection` and `sections`:
...
For plan-execute-review-commit flows: add plan API, `forge_createPlan` / `forge_executePlan` tools, `EditorReviewBar`. Reference: DialogueEditor and `packages/domain-forge`.
...
Related: how-to/07-assistant-and-ai, STATUS § Studio, components/01-editor-shell
