---
title: 24 - Verdaccio and local registry
created: 2026-02-07
updated: 2026-02-07
---

# 24 - Verdaccio and local registry

This guide gets the **local npm registry** up and running so you can publish `@forge/*` packages and install them from **another codebase** (e.g. your own app using the dev-kit) without publishing to public npm.

## What is Verdaccio?

**Verdaccio** is a lightweight, private npm registry that runs on your machine. We use it so that:

- Other codebases (or your own external app) can install `@forge/ui`, `@forge/shared`, `@forge/agent-engine`, and `@forge/dev-kit` from your local registry.
...
Config lives in the repo: [verdaccio.yaml](../../verdaccio.yaml) (storage under `./verdaccio/storage`, proxy to npmjs, `@forge/*` publish allowed).
...
Verdaccio runs at **http://localhost:4873**. Run it in a **separate terminal** (or in the background) when you are publishing or when another repo is installing `@forge/*` from this registry.

If the registry asks for login:
...
3. **Publish** from each package directory (one time or after changes):

   ```bash
   cd packages/ui && npm publish
   cd ../shared && npm publish
   cd ../agent-engine && npm publish
   cd ../dev-kit && npm publish
   ```
