...
- **Plan card (chat UI)**: `apps/studio/components/copilot/ForgePlanCard.tsx` -- renders `PlanCard` + `PlanActionBar` from `packages/shared/src/shared/copilot/generative-ui` so the plan shows inline in chat with Apply/Dismiss actions.
...
- **Plan**: `forge_createPlan(goal)` -> `createPlanApi(goal, graphSummary)` -> `POST /api/forge/plan` -> returns steps. The plan renders in chat via **PlanCard** with an Apply button. When applied, `forge_executePlan(steps)` runs each step via `applyOperations`, highlights, then `setPendingFromPlan(true)`. User sees **WorkspaceReviewBar**; Revert refetches graph, Accept clears pending; Save (or `forge_commit`) persists.
...
| Plan card | `packages/shared/.../copilot/generative-ui` + `apps/studio/components/copilot/ForgePlanCard.tsx` | Inline plan preview and Apply/Dismiss actions |
