---
title: 10 – Marketing site (customer-facing)
...
Same idea as editor components but lighter: atoms from `@forge/ui`, molecules and organisms in the app.
...
See `apps/marketing/components/README.md` and `AGENTS.md`. Marketing has its own `components.json` and registries for app-only components; shared UI (including shared billing components) comes from `@forge/ui` — see [15 – Shadcn registries and UI components](15-shadcn-registries-and-components.mdx) for details.

## Placeholder pages

The **Demo** (`/demo`), **Privacy** (`/privacy`), and **Terms** (`/terms`) pages are intentional placeholders (e.g. "Coming soon", "being updated") until final copy or assets are ready. The Hero "Watch Demo" CTA links to `/demo`. Replace placeholder content when legal or product has final copy.
