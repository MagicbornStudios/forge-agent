...
Forge (and any workspace that adopts it): create plan (no draft change) -> plan renders inline in chat via **PlanCard** with Apply/Dismiss -> execute plan (apply steps, set pending) -> **WorkspaceReviewBar** (Revert / Accept) -> commit (save). See Section 11 in [architecture/copilotkit-and-agents.md](../architecture/copilotkit-and-agents.md).
...
See also: [08 - Adding AI to workspaces](08-adding-ai-to-workspaces.md).
