...
## Strategy is per workspace

Strategy is applied **per workspace**, not per repo type. Each workspace (Forge, Video, Writer, Codebase Strategy) has its own strategy/artifacts/loop. This doc describes the **meta-strategy** (how we use artifacts across workspaces). Each workspace follows the same loop pattern but may have workspace-specific artifacts (e.g. Forge has graph-specific STATUS/decisions; Video has timeline-specific ones).
...
1. **Before a slice:** Read [agent-artifacts/core/STATUS.md](agent-artifacts/core/STATUS.md) and the relevant AGENTS.md (root + shared/workspace when touching workspaces). Use **rg** (or grep), **list_dir**/LS, and **Read** to locate and confirm files; check [agent-artifacts/core/errors-and-attempts.md](agent-artifacts/core/errors-and-attempts.md) before retrying a failed pattern. Understand current state and next.
2. **Prefer canonical artifacts:** Use the agent artifacts index and core artifacts (STATUS, decisions, errors-and-attempts) for **current state** and **failure log**. Use numbered docs (10–19, how-to, architecture) for architecture and how-to, not for live state. Avoid relying on stray READMEs for "what is true now."
3. **Before changing a pattern:** Use rg (or grep) to find occurrences; use list_dir/Read to confirm file locations. See [tool-usage.md](agent-artifacts/core/tool-usage.md).
4. **When changing persistence or data layer:** Read [agent-artifacts/core/decisions.md](agent-artifacts/core/decisions.md) and [11-tech-stack.mdx](11-tech-stack.mdx); update them when making or rejecting a significant choice.
5. **After a slice:** Update STATUS (including the Ralph Wiggum Done list) and any AGENTS/README that the change affects.
6. **When adding a new area with distinct rules:** Add an AGENTS.md and link from the index.
...
- **Toolchain integration:** In Cursor/Claude/VSCode, configure rules to "always include docs/18-agent-artifacts-index.mdx and root AGENTS.md (plus matching domain AGENTS.md) in context." Document that in your own runbook.
