...
1. **Before a slice:** Read [agent-artifacts/core/STATUS.md](agent-artifacts/core/STATUS.md) and the relevant AGENTS.md (root + shared/workspace when touching workspace types or panels). Use **rg** (or grep), **list_dir**/LS, and **Read** to locate and confirm files; check [agent-artifacts/core/errors-and-attempts.md](agent-artifacts/core/errors-and-attempts.md) before retrying a failed pattern. Understand current state and next.
...
7. **Scope of edits:** When implementing a slice, scope all file edits to the repo (apps, packages, docs). Do not modify files under `.tmp/` or other agent-download directories; they are reference material, not our code.
