...
1. **Before a slice:** Read [agent-artifacts/core/STATUS.md](agent-artifacts/core/STATUS.md) and the relevant AGENTS.md (root + shared/workspace when touching workspaces). Understand current state and next.
2. **Before retrying a failed approach:** Check [agent-artifacts/core/errors-and-attempts.md](agent-artifacts/core/errors-and-attempts.md). If the failure is documented, follow the fix instead of retrying the same pattern.
3. **When changing persistence or data layer:** Read [agent-artifacts/core/decisions.md](agent-artifacts/core/decisions.md) and [11-tech-stack.mdx](11-tech-stack.mdx); update them when making or rejecting a significant choice.
