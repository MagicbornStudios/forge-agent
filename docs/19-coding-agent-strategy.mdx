...
1. **Before a slice:** Read [agent-artifacts/core/STATUS.md](agent-artifacts/core/STATUS.md) and the relevant AGENTS.md (root + [packages/shared/src/shared/AGENTS.md](../packages/shared/src/shared/AGENTS.md) when touching shared editor or workspace types). Use **rg** (or grep), **list_dir**/LS, and **Read** to locate and confirm files; check [agent-artifacts/core/errors-and-attempts.md](agent-artifacts/core/errors-and-attempts.md) before retrying a failed pattern. Understand current state and next.
...
6. **After a slice:** Update STATUS (including the Ralph Wiggum Done list) and any AGENTS/README that the change affects. Ensure any new or edited doc under `docs/` has YAML frontmatter with at least **`title`** (string). When cleaning up dead code, run `pnpm knip` and address or document reported issues.
