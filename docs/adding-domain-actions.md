...
Every workspace that the agent operates should implement the shared **WorkspaceCapabilities** contract (see `src/shared/workspace/capabilities.ts`):
...
Actions are the way the agent changes the workspace. Define them in a single place per domain (e.g. `lib/copilot-actions.ts` for Forge). Each action has:
...
For a **new domain**, create a similar module (e.g. `lib/writer-copilot-actions.ts`) and optionally an **onAIChange** callback so the UI can highlight AI-created/edited elements.

## 4. Register actions when the workspace is active

Register actions only when that workspace is active. Forge does this by calling **useForgeCopilotActions** in `Workspace.tsx` with:
...
- **Forge actions** — `lib/copilot-actions.ts`, `lib/useForgeCopilotActions.ts`.
- **Capabilities** — `src/shared/workspace/capabilities.ts`.
