...
- **Override**: Client can send header `x-forge-model` (e.g. from workspace/editor/user setting for `ai.model`). When set, we use that model as primary and no fallbacks for that request.
...
- **Per-request**: Set `ai.model` at workspace/editor/user level to a specific model ID (not "auto"); the client sends `x-forge-model` and we use that model only for that request.
