...
| Surface | Purpose |
|--------|--------|
| **Header** | Project switcher, breadcrumbs, editor switcher (Left / Center / Right) |
...
- Inspector component (`EditorInspector`) receives `selection` and `sections: InspectorSection[]`.
...
- Pass `groups` to `EditorToolbar` to render from data; otherwise use `Left`/`Center`/`Right` slots.
...
- `EditorOverlaySurface` finds spec by `activeOverlay.id` and calls `render`. No file scanning or generators.
...
- **ModalRoute** / **ModalRegistry** - Legacy; prefer OverlaySpec + EditorOverlaySurface.
