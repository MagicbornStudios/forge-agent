---
title: CopilotKit and agents architecture
created: 2026-02-04
updated: 2026-02-07
---
...
## 1. App and mode hierarchy

- **Single Next.js app** (`apps/studio`): one layout, one CopilotKit provider, one App Shell.
- **App Shell** (`apps/studio/components/AppShell.tsx`): owns mode tabs and the active mode; only the active mode is mounted.
- **Modes**: Dialogue (primary, graph editor), Video (showcase), Character, Strategy (assistant-ui chat). Each uses `EditorShell` from shared and implements a **domain contract** (context + actions + suggestions + highlights).
- **Route**: Persisted in Zustand to localStorage (`forge:app-session:v2`); no URL for mode.
...
|--------|------------------|--------|
| Shell  | AppShell         | `useCopilotReadable`: `activeWorkspaceId`, `openWorkspaceIds`, mode labels, editor summary. |
...
| Layer   | Where | Actions |
|--------|-------|--------|
| Shell  | AppShell | `switchMode`, `openMode`, `closeMode` (prefixed via `createAppAction`). |
...
- **Review UI**: When `isDirty && pendingFromPlan`, a bar shows "Pending changes from plan" with **Revert** (refetch graph, clear pending) and **Accept** (keep draft, clear pending). The bar is the shared primitive `ModeReviewBar` from `@forge/shared/components/editor`; place it between toolbar and DockLayout.
...
| **Co-agents** | Documented, not used | Pattern in [17-co-agents-and-multi-agent.mdx](../17-co-agents-and-multi-agent.mdx). When needed: add `useCoAgent` (or equivalent) per mode/flow. |
...
Image generation (Section 8), structured output (Section 10), and plan-execute-review-commit (Section 11) are implemented. See [15-ai-workspace-integration.mdx](../15-ai-workspace-integration.mdx#missing--roadmap) and [agent-artifacts/core/STATUS.md](../agent-artifacts/core/STATUS.md) for status. **Product vs technical roadmap:** [docs/roadmap/](../roadmap/00-roadmap-index.mdx) - product roadmap for modes and initiatives; technical/infra items may be tracked there or separately. To add more AI (actions, agents, future graphs), see [08 - Adding AI to modes](../how-to/08-adding-ai-to-workspaces.mdx).

## 13. Conventions

- **One contract per domain**: All context/actions/suggestions/highlights go through `DomainCopilotContract`. Use `createAppAction` / `createDomainAction` for consistent prefixes.
