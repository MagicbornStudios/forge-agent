...
Single source of truth for how the AI sidebar, context, actions, and runtime work. See also [AI editor integration](../15-ai-workspace-integration.mdx) and [01-unified-workspace.mdx](./01-unified-workspace.mdx).
...
- **Editors**: Dialogue (primary, graph editor), Video (showcase), Character, Strategy (assistant-ui chat). Each uses `EditorShell` from shared and implements a **system contract** (context + actions + suggestions + highlights).
...
- **Review UI**: When `isDirty && pendingFromPlan`, a bar shows "Pending changes from plan" with **Revert** (refetch graph, clear pending) and **Accept** (keep draft, clear pending). The bar is the shared primitive `EditorReviewBar` from `@forge/shared/components/editor`; place it between toolbar and DockLayout.
