...
**Dockview integration:** The Dockview tab bar is customized to match `DockPanel` headers: each panel tab shows an icon and title using the same tokens (`--tab-height`, `--panel-padding`, `bg-card`, `border-border`). Active tab uses `--context-accent` (bottom border and optional glow). Pass the optional `slots` prop to override tab title and icon key per slot (e.g. `slots={{ main: { title: 'Dialogue Graphs' } }}`). Layout persistence, resize, and drag are provided by Dockview and are available to all consumers of `DockLayout`—including devkit users who use `DockLayout` + `DockPanel` from `@forge/shared/components/editor`.
...
|---|---|---|
| `DockLayout.tsx` | `DockLayout` | Resizable panel layout (Dockview) |
| `DockviewSlotTab.tsx` | (internal) | Custom Dockview tab (icons + tokens) |
