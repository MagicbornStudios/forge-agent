...
| **Used by** | Dialogue, Character, Video editors | Strategy editor (CodebaseAgentStrategyEditor) |
| **Provider** | `CopilotKit` wraps the entire app | `AssistantRuntimeProvider` wraps the Strategy editor |
| **Backend** | `/api/copilotkit` (CopilotKit runtime) | `/api/assistant-chat` (AI SDK chat transport) |
| **Model routing** | OpenRouter via OpenAI SDK + `x-forge-model` headers | OpenRouter via AI SDK `streamText` |
