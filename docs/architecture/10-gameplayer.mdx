...
- **In-browser:** Runtime runs in the browser (React + state machine or small interpreter). For MVP, "publish and host builds" (playable URL) can follow after first paid clone; the primary path is play inside Studio.
...
- **Publish and host:** Authors publish a "build" (playable bundle); we host it so others can play via URL. See STATUS § Next "Platform: publish and host builds."
