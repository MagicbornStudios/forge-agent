...
The chat UI operates the mode: the Copilot agent reads context from the active
mode and performs edits via **actions**. This doc describes the flow and how to
add or extend features per domain (Dialogue, Writer, etc.).

## Flow

1. **Context (what the agent sees)** - The mode exposes a snapshot via
   `useCopilotReadable`: `workspaceId`, `editorType`, domain-specific summary
...
2. **Actions (what the agent can do)** - Registered with `useCopilotAction`
   (or a domain hook like `useForgeCopilotActions`). Examples: create node,
...
3. **Optional: AI highlight** - When an action creates or edits an element, the
   mode can report affected ids via an `onAIChange` callback so the UI highlights
   them for a few seconds (or until the next user interaction).

## Single place for wiring

In each mode component (e.g. Dialogue `DialogueMode.tsx`), keep one clear block:

1. **Context** - `useCopilotReadable` with description and value (mode id,
   editor type, selection, summary).
2. **Action registration** - All CopilotKit actions for that domain
...
This keeps "AI -> mode" wiring in one place and makes it easy to add or
document new actions per domain.

## Per-domain action checklist

When adding or documenting a new mode/domain, consider implementing:
...
Not every domain needs all of these; implement what makes sense and document
the set for that domain.
...
See [architecture/03-copilotkit-and-agents.mdx](./architecture/03-copilotkit-and-agents.mdx) for full architecture, roadmap, and conventions (one contract per domain, `app_` prefix, new features as slices).

## References

- **Context + actions** - `apps/studio/components/modes/DialogueMode.tsx` (Dialogue), `packages/domain-forge/src/copilot`, `useDomainCopilot`.
...
- **Adding a new domain** - See [14-adding-domain-actions.mdx](./14-adding-domain-actions.mdx).
