...
All environment variables are defined in `scripts/env/manifest.mjs`. The manifest is the **source of truth**. Use `pnpm env:portal` to open the env portal UI, `pnpm env:sync:examples` to refresh `.env.example` files from the manifest.
