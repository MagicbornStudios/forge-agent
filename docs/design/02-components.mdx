...
Editors use **DockLayout** (Dockview) for full layout flexibility: resize splits, collapse panels, drag tabs to reorder, float panels. Layout is saved to `localStorage` on change and restored on load. **Reset layout:** clear `localStorage['dockview-{layoutId}']` for the editor (e.g. `dockview-dialogue-editor`) and reload.
