...
> **Rule of thumb:** import from `@forge/dev-kit` (or `@forge/ui` / `@forge/shared`) and keep domain code in your own app/system package.
...
**Avoid:** Unstyled grey buttons and text-only menu items. Reference screenshot: [buttons_dropdowns_menutitems_unstyled.png](../images/buttons_dropdowns_menutitems_unstyled.png). When updating UI patterns, add or update reference screenshots in **docs/images/** as needed. After applying contrast and spacing fixes (toolbar, PanelTabs, Dockview tabs, outline buttons), see [styling_after_contrast_and_spacing.png](../images/styling_after_contrast_and_spacing.png) when available.
...
Editors use **DockLayout** (Dockview) for full layout flexibility: resize splits, collapse panels, drag tabs to reorder, float panels. Layout is saved to `localStorage` on change and restored on load. **Reset layout:** clear `localStorage['dockview-{layoutId}']` for the editor (e.g. `dockview-dialogue-editor`) and reload. Dockview tabs are themed to match DockPanel headers (icons, `--tab-height`, active state with `--context-accent`). Docking, resize, and persist are available to end users and to devkit consumers who use `DockLayout` and `DockPanel`.
