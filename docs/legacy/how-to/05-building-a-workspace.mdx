...
Create `MyEditor.tsx`. Compose `EditorShell`, `EditorHeader`, `EditorToolbar`,
`DockLayout` (with a placeholder `main`), `EditorStatusBar`, `EditorOverlaySurface`.
No data binding yet.
...
Inspector (`EditorInspector`). Pass `selection` and `sections` (or children). Selection
...
and `EditorReviewBar` between Toolbar and DockLayout. Render the plan in chat with
