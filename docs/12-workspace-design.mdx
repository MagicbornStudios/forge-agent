...
1. **New editor**: Add an editor id to `EditorId` in `lib/app-shell/store.ts`. Create an editor component (e.g. `XEditor`) using `EditorShell`, wire system store, `useXContract` + `useDomainCopilot`. Add tab and render branch in `AppShell`. Register shell context/actions (editor name, switch/open/close) if not already generic.
