...
- **Context budget:** Reference DECISIONS (Context budget and compaction). 128k per request; summarize chat/tool results beyond recent ~15 turns; fixed planning list + on-demand retrieval. Enables long runs and overnight mode without context overflow.
- **Indexing exclusions:** Reference DECISIONS (Codebase indexing and exclusion policy). .gitignore + vendor/ (default) + .cursorignore; opt-in vendor.
