...
Workspace Platform Engineer: owns `src/shared/components/workspace` and shared workspace models.
...
Add components via: `npx shadcn@latest add <name>`. Use shadcn atoms from `components/ui/*` (Button, Separator, Card, ScrollArea, Dialog, DropdownMenu, Select, Sheet, Tabs, Popover, Menubar, etc.). Do not hand-roll equivalents.
...
- **No cross-domain imports**: Shared components do not import from app or domain code.
